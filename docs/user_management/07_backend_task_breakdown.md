...
目标：提升 API Key 签发可用性并固化组织/成员一致性约束，支持前端成员联想选择。

1. BE-6.1 成员检索接口（关联 TD-502 / BC-501）
- 新增 `GET /api/admin/member-options`，支持 `query/orgId/limit`。
- 返回最小候选字段，避免泄露非必要信息。
2. BE-6.2 成员检索仓储实现（关联 TD-502 / BC-501）
- 按组织 + 邮箱/显示名前缀查询 `memberships + users`。
- 固定 `limit` 上限并补齐必要索引/查询优化。
3. BE-6.3 签发接口组织语义增强（关联 TD-503/TD-505 / BC-502）
- `POST /api/admin/api-keys` 支持 `orgId?`。
- 严格校验 `orgId` 与 `userId` membership 归属，拒绝跨组织组合。
4. BE-6.4 API Key 列表可读字段扩展（关联 TD-504 / BC-503）
- `GET /api/admin/api-keys` 响应增加 `userEmail/userDisplayName`。
- 保持旧字段兼容，不破坏现有调用。
5. BE-6.5 错误码与审计收口（关联 TD-503 / BC-502）
- 跨组织/目标不存在场景统一返回 `PERMISSION_DENIED/RESOURCE_NOT_FOUND`。
- 所有拒绝路径写 `FAILED` 审计并记录组织与目标信息。
6. BE-6.6 自动化测试补齐（关联 TD-501~TD-505）
- 覆盖成员检索、签发组织校验、列表字段扩展、兼容与回归场景。
...
8. BE-6.x 依赖 BE-5.x 上下文能力稳定；并被 FE-6.x（签发联想）直接消费。
...
6. Phase 6 先落成员检索契约与接口，再增强签发组织校验，最后补齐列表字段与测试。
...
7. 风险：成员检索接口被滥用引发性能抖动。
- 缓解：查询最小长度、固定 `limit` 上限、前缀匹配优先索引路径。
...
| BE-6.1 | TD-502 | BC-501 |
| BE-6.2 | TD-502 | BC-501 |
| BE-6.3 | TD-503/TD-505 | BC-502 |
| BE-6.4 | TD-504 | BC-503 |
| BE-6.5 | TD-503 | BC-502 |
| BE-6.6 | TD-501~TD-505 | BC-501/BC-502/BC-503 |
