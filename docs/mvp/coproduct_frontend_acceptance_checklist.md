...
## 4.5 M3.1 导航与跨页面跳转（前端-only）

1. 顶部全局导航在首页/新建页/详情页/历史页均可见
2. 导航支持 `首页/新建预审/历史记录` 的直接跳转
3. 当前路由有高亮标识，用户可识别所在页面
4. 详情页提供“返回历史”“新建预审”快捷操作
5. `/review/*` 旧链接重定向行为仍保持正确
6. 该验收项不依赖后端新增接口
